...
void convertToNote(float freq, std::string *Note, int *Octave) {
    ...
    *Octave = stepsAway / 12 + 4;  // Calculate the octave
    ...
    else if (steps < 0) *Note = "C"; //in rare ocasions if the freq to 261 is close enough, a negative value can be output
...
int main() {
    ...
    const char *filename = "Happ48k.wav";  // Input audio file

    std::ofstream csvFile("YIN.csv");
    ...
    uint_t buf_size = 512;     // Buffer size
    ...
    aubio_pitch_t *pitch_detector = new_aubio_pitch("yin", buf_size, hop_size, samplerate);
